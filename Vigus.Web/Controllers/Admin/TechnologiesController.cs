...
public class TechnologiesController : Controller
{
    ...
    {
        if (id == null || _context.GpuTechnologies == null) return NotFound();

        var gpuTechnology = await _context.GpuTechnologies
            .Include(g => g.Image)
            .FirstOrDefaultAsync(m => m.Id == id);
        if (gpuTechnology == null) return NotFound();
    ...
    {
        if (id != gpuTechnology.Id) return NotFound();
    ...
    {
        if (_context.GpuTechnologies == null) return Problem("Entity set 'VigusGpuContext.GpuTechnologies'  is null.");
        var gpuTechnology = await _context.GpuTechnologies.FindAsync(id);
        if (gpuTechnology != null) _context.GpuTechnologies.Remove(gpuTechnology);
