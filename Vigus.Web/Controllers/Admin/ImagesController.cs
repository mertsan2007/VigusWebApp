...
public class ImagesController : Controller
{
    ...
    {
        if (id == null || _context.Images == null) return NotFound();
    ...
    public async Task<IActionResult> Create([Bind("Title,File,Gpus,Technologies,Id")] Image image)
    {
        if (ModelState.IsValid)
        {
            ...
            var path = Path.Combine(rootPath + "/Images/UserUploads", image.Name);
    ...
    {
        if (_context.Images == null) return Problem("Entity set 'VigusGpuContext.GpuImages'  is null.");
        var image = await _context.Images.FindAsync(id);
